...
<template>
  <section id="screen-clock" class="screen fill flex" :class="{ active: state.current === 'clock', leaving: state.leaving === 'clock', timer: mode === 'timer' }">
    <LeftRail v-bind="mode === 'timer' ? { eyebrow: timerLabel, title: timerTitle, subtitle: time.time } : { eyebrow: CT.MONTHS[parts.month], title: parts.date, subtitle: CT.DAYS[parts.day] }">
      <template #lower>
